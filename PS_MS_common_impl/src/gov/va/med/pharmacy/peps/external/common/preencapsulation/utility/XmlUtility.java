...
public class XmlUtility {
    ...
    protected static <T> T unmarshal(String xml, boolean validate, JAXBContext jaxbContext, String schemaFile,
            Class<T> returnType) {
        InputStream schemaInput = Thread.currentThread().getContextClassLoader().getResourceAsStream(schemaFile);
        return unmarshal(xml, validate, jaxbContext, schemaInput, returnType);
    }
     ...
     */
    protected static <T> T unmarshal(String xml, boolean validate, JAXBContext jaxbContext, InputStream schemaInput,
                                     Class<T> returnType) {
        ...
        try {
            ...
            if (validate) {
                ...
                synchronized (SCHEMA_FACTORY) {
                    Schema schema = SCHEMA_FACTORY.newSchema(new StreamSource(schemaInput));
